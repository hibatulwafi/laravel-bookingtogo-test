...
<div class="container">
    <h1>Edit Customer</h1>
    <form action="{{ route('customers.update', $customer->cst_id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group">
            <label for="name">Nama:</label>
            <input type="text" class="form-control" name="cst_name" id="name" value="{{ $customer->cst_name }}" required>
        </div>
        <div class="form-group">
            <label for="dob">Tanggal Lahir:</label>
            <input type="date" class="form-control" name="cst_dob" id="dob" value="{{ $customer->cst_dob }}" required>
        </div>
        <div class="form-group">
            <label for="phone">Phone Number:</label>
            <input type="text" class="form-control" name="cst_phoneNum" id="phone" value="{{ $customer->cst_phoneNum }}" required>
        </div>
        <div class="form-group">
            <label for="email">Email:</label>
            <input type="email" class="form-control" name="cst_email" id="email" value="{{ $customer->cst_email }}" required>
        </div>
        <div class="form-group">
            <label for="nationality">Kewarganegaraan:</label>
            <select class="form-control" name="nationality_id" id="nationality" required>
                @foreach($nationalities as $nationality)
                <option value="{{ $nationality->nationality_id }}" {{ $nationality->nationality_id == $customer->nationality_id ? 'selected' : '' }}>{{ $nationality->nationality_name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div id="family-container">
            ...
            <div class="form-group family">
                <div class="row">
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="family_name[]" value="{{ $family->fl_name }}" placeholder="Nama Keluarga" required>
                    </div>
                    <div class="col-md-4">
                        <input type="date" class="form-control" name="family_dob[]" value="{{ $family->fl_dob }}" placeholder="Tanggal Lahir Keluarga" required>
                    </div>
                    <div class="col-md-4">
        ...
        <button type="submit" class="btn btn-success" style="margin-top: 20px;">Simpan Perubahan</button>
    </form>
</div>
